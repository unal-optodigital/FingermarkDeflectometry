...
def phase_compensation(wrappedphase):
    ...
    slider1 = Slider(slider_ax1, 'x', -4,0, valinit=0.05, valstep=0.00001,orientation='vertical')
    slider_ax5=plt.axes([0.15, 0.25, 0.03, 0.65])
    slider5=Slider(slider_ax5, 'offset', -2, 2, valinit=0, valstep=0.0001,orientation='vertical')
    slider_ax2 = plt.axes([0.1, 0.25, 0.03, 0.65])
    slider2 = Slider(slider_ax2, 'y', -2,2, valinit=0, valstep=0.0001,orientation='vertical')
    slider_ax3=plt.axes([0.8, 0.25, 0.03, 0.65])
    slider3=Slider(slider_ax3, 'x^2', -0.5, 0.5, valinit=0, valstep=0.00001,orientation='vertical')
    slider_ax4=plt.axes([0.04, 0.25, 0.03, 0.65])
    slider4=Slider(slider_ax4, 'y^2', -0.5, 0.5, valinit=0, valstep=0.00001,orientation='vertical')
...
def phasor(array_of_frames):
    ...
    # normalization = [im for im in array_of_frames] 
...
def run_camera_and_fringes_ui(
    freq_ini: float,
    second_screen_width: int,
    second_screen_height: int,
    displacement_x: int,   # move 1920 pixels the second screen
    displacement_y: int,
    monitors_size: list[list[int]],
    main_monitor_size: list,
    cam_exposure: float,
    slider_fig_size=(5, 2),):
    """
    ...
    """
    main_screen_width = main_monitor_size[0]
    main_screen_height = main_monitor_size[1]
    # camera_resolution = (2848, 2848)
    # print("scale:", scale)
    # print("new size:", ((int(scale*camera_resolution[0]), int(scale*camera_resolution[1]))))
    # geometrý 
    # Meshgrid

    if monitors_size[1][0] !=0 and monitors_size[1][1] !=0:
        # print("resolution, second monitor parameters")
        width_monitor_m, height_monitor_m = (monitors_size[1][0])/1000, (monitors_size[1][1])/1000
        main_monitor_width_m,  main_monitor_height_m = (monitors_size[0][0])/1000, (monitors_size[0][1])/1000
    else:
        # print("resolution, first monitor parameters")
        width_monitor_m, height_monitor_m = (monitors_size[0][0])/1000, (monitors_size[0][1])/1000
        main_monitor_width_m,    = (monitors_size[0][0])/1000, (monitors_size[0][1])/1000


    width_pixel_size, height_pixel_size = width_monitor_m/second_screen_width , height_monitor_m/second_screen_height

    x = width_pixel_size * np.arange(0,second_screen_width, 1)
    y= height_pixel_size * np.arange(0,second_screen_height, 1)
    X, Y = np.meshgrid(x, y)
    ...
    state = {
        'freq': float(freq_ini),
        'angle': 0.0,
        'exposure_time': cam_exposure,  # µs initial value (10 ms)
        'stop': False,
        'cam': None,               
    }
    ...
    fig_sliders.canvas.manager.set_window_title('Fringe Controls (CLOSE TO CONTINUE)')
    ...
    slider_freq = Slider(ax_freq, 'frequency', valmin=0.0, valmax=500.0,
                         valinit=freq_ini, valstep=0.05)
    slider_angle = Slider(ax_angle, 'angle', valmin=0.0, valmax=2*np.pi,
    ...
    cv2.resizeWindow(WIN_PATTERN, second_screen_width, second_screen_height)
    cv2.moveWindow(WIN_PATTERN, displacement_x, displacement_y)
    cv2.setWindowProperty(WIN_PATTERN, cv2.WND_PROP_FULLSCREEN, cv2.WINDOW_FULLSCREEN)


    # camera window
    WIN_CAMERA = 'camera'
    cv2.namedWindow(WIN_CAMERA, cv2.WINDOW_NORMAL | cv2.WINDOW_KEEPRATIO)
    cv2.resizeWindow(WIN_CAMERA, main_screen_width-50, main_screen_height-50)
    cv2.moveWindow(WIN_CAMERA, 0, 0)
            ...
            try:
                while not state['stop']:
                    ...
                    try:
                        ...
                        cv2.resize(img, ((int(scale*camera_resolution[0]), int(scale*camera_resolution[1]))), interpolation=cv2.INTER_AREA)
...
def crop_image(img: np.ndarray) -> np.ndarray:
    ...
    # if close the window, the coords will be saved in roi_coords
...
def partition_in_boxes(h: int, w: int, cols_per_row=(5,5,5,5,5)):
    """
    Divides a image (h,w) in boxes cols_per_row.
    for 3x3: cols_per_row=(3,3,3) -> 9 regions.
    Returns list of bounds (y0,y1,x0,x1) in order row-major.
    """
    cols_per_row = list(cols_per_row)
    if any(c < 1 for c in cols_per_row):
        raise ValueError("cols_per_row Error")
...
def evaluate_and_detect_monitors():
    """
    Use the information of the monitors detected
    returns:  main monitor resolution (X,X), window_displacement, resize_window_height, resize_window_width, monitors_size_mm
    ...
    try:
        ...
        resize_window_width = int(second_monitor.width)
        window_displacement = int(main_monitor.width)
        second_monitor_height_mm =int(second_monitor.height_mm)
        second_monitor_width_mm= int(second_monitor.width_mm)

    except NameError:
        print('Second monitor not detected')
        resize_window_height = int(main_monitor.height)
        resize_window_width = int(main_monitor.width)
        window_displacement = int(0)
        second_monitor_height_mm =int(0)
        second_monitor_width_mm= int(0)
    main_monitor_height_mm =int(main_monitor.height_mm)
    main_monitor_width_mm = int(main_monitor.width_mm)
    # tamaño del resized para la visualización del levantamiento + canvas
    main_monitor_size = (int(main_monitor.width), int(main_monitor.height))
    
    monitors_size_mm = [[main_monitor_width_mm, main_monitor_height_mm],[second_monitor_width_mm,second_monitor_height_mm]]
    return main_monitor_size, window_displacement, resize_window_height, resize_window_width, monitors_size_mm


# # fingermark va desde 1 hasta N, en total N fingermarks
# fingermark = 21
# linea_to_be_replaced = fingermark
# reemplazar_linea(r'C:\Users\Perez\OneDrive\Escritorio\Github\FingerprintDeflectometry\prueba_linea.txt', linea_to_be_replaced, f"{fingermark} " + f"{20}")
